...
type ReportRow = {
  ...
  role: string
  // Negotiated settlement contract fields (6 decimal royalty points).
  C_future: number | null
  T_future: number | null
  B_future: number | null
  C_past: number | null
  T_past: number | null
  B_past: number | null
...
type SortKey =
  | 'name' | 'group' | 'role'
  | 'C_future' | 'T_future' | 'B_future' | 'C_past' | 'T_past' | 'B_past'
  | 'value_or_cost' | 'raw_score' | 'notes' | 'edit'

function decValue(r: ReportRow, key: SortKey): number | null {
  return (r as unknown as Record<string, number | null>)[key]
}

function decCol(key: SortKey, label: string): SortableColumn<ReportRow, SortKey> {
  return {
    key, label, nullsLast: true,
    isNull: r => decValue(r, key) === null,
    tiebreak: (a, b) => a.display_name.localeCompare(b.display_name),
    render: r => <span style={{ fontVariantNumeric: 'tabular-nums' }}>{fmtDec(decValue(r, key))}</span>,
    compare: (a, b) => (decValue(a, key) ?? 0) - (decValue(b, key) ?? 0),
  }
}
...
const COLUMNS: readonly SortableColumn<ReportRow, SortKey>[] = [
  ...
  decCol('C_future', 'C future'),
  decCol('T_future', 'T future'),
  decCol('B_future', 'B future'),
  decCol('C_past',   'C past'),
  decCol('T_past',   'T past'),
  decCol('B_past',   'B past'),
  {
    key: 'value_or_cost', label: 'Value / Cost', nullsLast: true, isNull: r => r.value_or_cost === null,
    tiebreak: (a, b) => a.display_name.localeCompare(b.display_name),
    render: r => <span style={{ fontVariantNumeric: 'tabular-nums' }}>{fmtDec(r.value_or_cost)}</span>,
...
export default function Reports() {
  ...
  //    This is also where Gary enters simulated court outcomes for no-deal groups. ──
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '1rem 1.5rem 0.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
        ...
        <h2 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 600 }}>Reports — Claridge</h2>
      </div>
