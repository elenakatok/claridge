...
import { claridgeConfig, claridgeSchema, FIELD_LABELS, formatField } from '../gameConfig'
...
// ── Claridge-specific outcome formatter ───────────────────────────────────────

function formatClaridgeOutcome(
  ...
  if (!agreementReached || outcome == null) {
    return (
      <p style={{ fontSize: '1.05rem', color: colors.textSecondary, marginBottom: layout.pagePad }}>
        No deal reached — the case goes to court.
      </p>
    )
  }
  return (
  ...
  if (session.kind === 'no-token') {
    return (
      <main style={{ padding: '2rem', fontFamily: typography.fontFamily, maxWidth: '480px', margin: '2rem auto' }}>
        <h2 style={{ marginBottom: '0.75rem' }}>Claridge</h2>
        <p>Please launch Claridge from the classroom to join a session.</p>
    ...
    <div style={{ fontFamily: typography.fontFamily }}>
      ...
      {phase.name === 'off-platform' && (
        <>
          <div style={{ maxWidth: layout.contentWidth, margin: '0 auto', paddingTop: layout.pagePad, paddingLeft: layout.pagePad, paddingRight: layout.pagePad }}>
            <GroupMembersPanel groupId={phase.groupId} participantId={participantId} gameInstanceId={gameInstanceId} roleConfig={claridgeConfig} db={db} rtdb={rtdb} functions={functions} />
          </div>
          <OffPlatformHolding
            groupId={phase.groupId}
            participantId={participantId}
      ...
      {phase.name === 'results' && (
        <Results
          ...
          gameInstanceId={gameInstanceId}
          roleConfig={claridgeConfig}
          formatOutcome={formatClaridgeOutcome}
          db={db}
          rtdb={rtdb}
          functions={functions}
        />
