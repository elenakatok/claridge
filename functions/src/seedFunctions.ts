...
type ClaridgeRole = 'claridge' | 'tolemite' | 'bard'
...
export const seedMatchTest = onRequest(async (req, res) => {
  ...
  type SeedP = { id: string; role: ClaridgeRole }
...
// Claridge group composition: 1 claridge + 1 tolemite + 1 bard = 3 participants.
export const seedGroupForTest = onRequest(async (req, res) => {
  ...
  const body = (req.body?.data ?? req.body) as {
    ...
    group_id?: unknown
    lead_id?: unknown
    claridge_participants?: unknown
    tolemite_participants?: unknown
    bard_participants?: unknown
  }
  ...
  if (!Array.isArray(body.claridge_participants) || !Array.isArray(body.tolemite_participants) || !Array.isArray(body.bard_participants)) {
    res.status(400).json({ error: 'claridge_participants, tolemite_participants and bard_participants arrays required' }); return
  }

  const gameInstanceId = body.game_instance_id
  const groupId = body.group_id
  const leadId = body.lead_id
  const byRole: Record<ClaridgeRole, string[]> = {
    claridge: body.claridge_participants as string[],
    tolemite: body.tolemite_participants as string[],
    bard:     body.bard_participants as string[],
  }
  ...
  await groupRef.set({
    ...
    claridge_participants: byRole.claridge,
    tolemite_participants: byRole.tolemite,
    bard_participants: byRole.bard,
    lead_participant_id: leadId, outcome: null, status: 'matched', matched_at: now,
  })

  const batch = db.batch()
  for (const role of ['claridge', 'tolemite', 'bard'] as ClaridgeRole[]) {
